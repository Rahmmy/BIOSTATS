...
The Jaccard distance provides more similarity that the correlation based distance


Other distance calcuylation functions are;
...
Flow cytometry (FC) is a technique used to detect and measure physical and chemical characteristics of a population of cells or particles.  Flow cytometry is routinely used in basic research, clinical practice, and clinical trials. Uses for flow cytometry include:
...
```{r eval=TRUE}
#load the table data that reports the mapping between isotopes and markers (antibodies)
...
#plto data using flowPlot function from flowViz
...
dadaFs1 = dada(derepFs, err=ddF[[1]]$err_out, pool = TRUE)
dadaRs1 = dada(derepRs, err=ddR[[1]]$err_out, pool = TRUE)
...
origAddress <- origAddress <- read.xlsx(here("Book","data","Location.xlsx"))#, stringsAsFactors = FALSE)
